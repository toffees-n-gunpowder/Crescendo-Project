...
def record_play_once(user_id, track_id, window_seconds=30):
    ...
    """
    return core.scalar('SELECT record_play(%s, %s, %s)',
                       [user_id, track_id, window_seconds])


def get_play_history(user_id, limit=50):
    return core.query('SELECT * FROM get_play_history(%s, %s)', [user_id, limit])
...
def delete_group(group_id, owner_id):
    ...
    core.execute('CALL delete_group_proc(%s, %s)', [group_id, owner_id])
    return 1


def add_group_review(group_id, reviewer_id, rating, review_text):
    core.execute('CALL save_group_review(%s, %s, %s, %s)',
                 [group_id, reviewer_id, rating, review_text])
